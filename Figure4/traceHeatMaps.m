...
allBefTraces = zeros(numel(befTraces), numel(befTraces(1).file(1).dftrace));
for idx = 1:numel(befTraces)
    allBefTraces(idx,:) = 100 * befTraces(idx).file(1).dftrace; %Multiply by 100 to get percent
end
%After Blast Reshaping
allAftTraces = zeros(numel(aftTraces(1).file), numel(aftTraces), numel(aftTraces(1).file(1).dftrace));
for fiter = 1:numel(aftTraces(1).file) %File Iteration
    for idx = 1:numel(aftTraces) %Cell Iteration
        allAftTraces(fiter,idx,:) = 100 * aftTraces(idx).file(fiter).dftrace; %Multiply by 100 to get percent
    end
end

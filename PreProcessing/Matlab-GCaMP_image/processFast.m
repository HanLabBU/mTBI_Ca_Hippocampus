...
for n=1:numel(tifFile) % -- Hua-an
    tifFile(n).firstIdx = fileFrameIdx.first(n);
    tifFile(n).lastIdx = fileFrameIdx.last(n);
end

% [tifFile.firstIdx] = deal(fileFrameIdx.first); % no working -- Hua-an
% [tifFile.lastIdx] = deal(fileFrameIdx.last); % no working -- Hua-an
% ------------------------------------------------------------------------------------------
% PROCESS FIRST FILE
% ------------------------------------------------------------------------------------------
[d8a, singleFrameRoi, procstart, info, original_data] = processFirstVidFile(tifFile(1).fileName);
...
allVidFiles(1) = vfile;
original_vfile = saveOriginalVidFile(original_data,info, tifFile(1));
original_allVidFiles = VideoFile.empty(nFiles,0);
original_allVidFiles(1) = original_vfile;
...
for kFile = 2:numel(tifFile)
   ...
   [f.d8a, f.singleFrameRoi, procstart, f.info, f.original_data] = processVidFile(fname, procstart);
   ...
   original_vfile = saveOriginalVidFile(f.original_data, f.info, tifFile(kFile));
   original_allVidFiles(kFile,1) = original_vfile;
   %    d8a = cat(3,d8a, f.d8a);
...
% ------------------------------------------------------------------------------------------
% Fix the first and last frame -- Hua-an
% ------------------------------------------------------------------------------------------

if numel(tifFile)~=numel(allVidFiles) || numel(tifFile)~=numel(original_allVidFiles)
    fprintf('Insistent file number!\n');
else
    for n=1:numel(tifFile)
        allVidFiles(n).total_first_frame = tifFile(n).firstIdx;
        allVidFiles(n).total_last_frame = tifFile(n).lastIdx;
        original_allVidFiles(n).total_first_frame = tifFile(n).firstIdx;
        original_allVidFiles(n).total_last_frame = tifFile(n).lastIdx;
    end
end
...
roiFileName = ...
   ...
   datestr(saveTime,'yyyy_mm_dd_HHMM'),...
   '.mat'];
processedOriginalVidFileName =  ...
   ['Processed_OriginalVideoFiles_',...
   uniqueFileName,'_',...
   datestr(saveTime,'yyyy_mm_dd_HHMM'),...
   '.mat'];
SingleFrameRoiFileName =  ...
   ['SingleFrame_ROIs_',...
   uniqueFileName,'_',...
   datestr(saveTime,'yyyy_mm_dd_HHMM'),...
   '.mat'];
...
save(fullfile(fdir, processedOriginalVidFileName), 'original_allVidFiles');
...
% ------------------------------------------------------------------------------------------
fprintf('Start merging single frame ROIs....\n');
singleFrameRoi = fixFrameNumbers(singleFrameRoi);
try
    save(fullfile(fdir, SingleFrameRoiFileName), 'singleFrameRoi', '-v7.3');
catch
    fprintf('Error in saving single frame ROIs!\n');
end
...
R.check_framesize; % make sure the frame size is consistant

save(fullfile(fdir, CombinedRoiFileName), 'R', '-v6');
save('allROI','R')


% fprintf('Find top 1000 ROIs....\n');
% fds = zeros(numel(R),1);
% for k=1:numel(R)
...
% ------------------------------------------------------------------------------------------
% MAKE BACKGROUD MASK AND CHECK IF THE ROI IS ISOLATED -- Hua-an
% ------------------------------------------------------------------------------------------
background_range = 3; % the size of outline in pixel
isolate_range = 1; % the min distance between ROIs
background_se = strel('disk', background_range, 0);
isolate_se = strel('disk', isolate_range, 0);
all_ROI_mask = zeros(R(1).FrameSize);
all_ROI_mask(unique(cat(1,R.PixelIdxList)))=1;
for k=1:numel(R)
    R(k).id = k;
    ROI_mask = zeros(R(k).FrameSize);
    ROI_mask(R(k).PixelIdxList)=1;
    ROI_background = imdilate(ROI_mask,background_se);
    ROI_background = (ROI_background==1) & (all_ROI_mask~=1);
    ROI_isolate = imdilate(ROI_mask,isolate_se);
    ROI_isolate = (ROI_isolate==1) & (ROI_mask~=1);
    if isempty(find(ROI_isolate==1 & all_ROI_mask==1,1))
        R(k).isIsolate = true;
    else
        R(k).isIsolate = false;
    end
    R(k).BackgroundRange = background_range;
    R(k).BackgroundPixelIdxList = find(ROI_background==1);
    R(k).BackgroundArea = numel(R(k).BackgroundPixelIdxList);
    R(k).ROIBackgroundRatio = R(k).Area/R(k).BackgroundArea;
end

fprintf(['Isolated ROIs(at ',num2str(isolate_range),' pixel distance): ',num2str(numel(findobj(R,'isIsolate',true))),'\n']);

% ------------------------------------------------------------------------------------------
% RELOAD EACH FILE AND MAKE ROI TRACES -- Hua-an 
% ------------------------------------------------------------------------------------------

if allVidFiles(end).total_last_frame~=original_allVidFiles(end).total_last_frame
    fprintf('Inconsistant total frame number!\n');
end

total_frame_number = allVidFiles(end).total_last_frame;

set(R,'TraceTime',zeros(total_frame_number,1));
set(R,'Trace',zeros(total_frame_number,1));
set(R,'TraceWithoutWeight',zeros(total_frame_number,1));
set(R,'RawTrace',zeros(total_frame_number,1));
set(R,'RawTraceWithoutWeight',zeros(total_frame_number,1));
set(R,'BackgroundTrace',zeros(total_frame_number,1));
set(R,'RefinedTrace',zeros(total_frame_number,1));

for n=1:numel(allVidFiles)
    [data, vidinfo] = getData(allVidFiles(n));
    data = squeeze(data);
    X = makeTraceFromVid(R,data);
    X_WithoutWeight = makeTraceFromVidWithoutWeight(R,data);
    clear data;
    
    [data, vidinfo] = getData(original_allVidFiles(n));
    data = squeeze(data);
    original_X = makeTraceFromVid(R,data);
    original_X_WithoutWeight = makeTraceFromVidWithoutWeight(R,data);
    background_X = makeBackgroundTraceFromVid(R,data);
    
    for k=1:numel(R)
       R(k).TraceTime(allVidFiles(n).total_first_frame:allVidFiles(n).total_last_frame) = vidinfo.time;
       R(k).Trace(allVidFiles(n).total_first_frame:allVidFiles(n).total_last_frame) = X(:,k);
       R(k).TraceWithoutWeight(allVidFiles(n).total_first_frame:allVidFiles(n).total_last_frame) = X_WithoutWeight(:,k);
       R(k).RawTrace(allVidFiles(n).total_first_frame:allVidFiles(n).total_last_frame) = original_X(:,k);
       R(k).RawTraceWithoutWeight(allVidFiles(n).total_first_frame:allVidFiles(n).total_last_frame) = original_X_WithoutWeight(:,k);
       R(k).BackgroundTrace(allVidFiles(n).total_first_frame:allVidFiles(n).total_last_frame) = background_X(:,k);
       R(k).RefinedTrace(allVidFiles(n).total_first_frame:allVidFiles(n).total_last_frame) = original_X_WithoutWeight(:,k) - background_X(:,k);
    end
...
% % ------------------------------------------------------------------------------------------
% % RELOAD DATA AND MAKE ROI TRACES (REMOVED: NORMALIZED TO WINDOWED STD)
% % ------------------------------------------------------------------------------------------
% 
% 
% 
...
% 
% % ------------------------------------------------------------------------------------------
% % RELOAD DATA AND MAKE ORIGINAL ROI TRACES AND BACKGROUND TRACE
% % ------------------------------------------------------------------------------------------
% 
% [data, vidinfo] = getData(original_allVidFiles);
% data = squeeze(data);
% original_X = makeTraceFromVid(R,data);
% original_X_WithoutWeight = makeTraceFromVidWithoutWeight(R,data);
% background_X = makeBackgroundTraceFromVid(R,data);
% 
% for k=1:numel(R)
%    R(k).TraceTime = vidinfo.time;
%    R(k).Trace = X(:,k);
%    R(k).TraceWithoutWeight = X_WithoutWeight(:,k);
%    R(k).RawTrace = original_X(:,k);
%    R(k).RawTraceWithoutWeight = original_X_WithoutWeight(:,k);
%    R(k).BackgroundTrace = background_X(:,k);
%    R(k).RefinedTrace = R(k).RawTraceWithoutWeight - R(k).BackgroundTrace;
% end
% 
...
% ------------------------------------------------------------------------------------------
try
    fprintf(['Save ROIs with trace: ',roiFileName,'\n']);
    save(fullfile(fdir,roiFileName), 'R', '-v7.3');
    fprintf(['Save ROIs with trace....DONE!\n']);
catch
    fprintf('Error in saving ROIs!\n');
end
...
% SUBFUNCTIONS
% ################################################################
function [d8a, singleFrameRoi, procstart, info, original_data] = processFirstVidFile(fname)
% LOAD FILE
...
original_data = uint8(data .* (255/65535));
% FILTER AGAIN
...
function [d8a, singleFrameRoi, procstart, info, original_data] = processVidFile(fname, procstart)
% LOAD FILE
[data, info, tifFile] = loadTif(fname);
% GET COMMON FILE-/FOLDER-NAME
nFiles = numel(tifFile);
...
function original_vfile = saveOriginalVidFile(data,info,tifFile)
[expDir, expName] = fileparts(tifFile.fileName);
original_vfile = VideoFile('rootPath',expDir,'experimentName',expName,'bitDepth',16);
vfinfo.FrameNumber = cat(1,info.frame);
vfinfo.time = cat(1,info.t);
original_vfile.addFrame2File(data,vfinfo);
vidFileDir = [expDir, '\', 'original_VidFiles'];
if ~isdir(vidFileDir)
   mkdir(vidFileDir);
end
save(fullfile(vidFileDir,expName), 'original_vfile');
end

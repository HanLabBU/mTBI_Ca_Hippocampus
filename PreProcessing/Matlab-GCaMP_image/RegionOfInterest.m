classdef RegionOfInterest < hgsetget %& dynamicprops & matlab.mixin.CustomDisplay
   ...
   properties % move RawTrace to Hua-an
	  PixelSubScripts
	  PixelWeights
	  PixelCounts
	  BoundaryTrace
   ...
   properties % Hua-an
      RawTrace
      TraceWithoutWeight
      RawTraceWithoutWeight
      TraceTime
      BackgroundRange
      BackgroundArea
      BackgroundPixelIdxList
      BackgroundTrace % without weight
      ROIBackgroundRatio
      RefinedTrace
      isIsolate = false
      id
		 ...
		 for kObj = 1:N
			...
			if isnumeric(propval)
			   if abs(propval-round(propval)) < eps
				  propvalstring = sprintf('%5.0f ',propval);
			   else
...
%          delete(gcp)
   ...
   methods % DATA GENERATION, STORAGE, & RETRIEVAL
	  function varargout = makeTraceFromVidLowMem(obj, vid)
...
% 		 end
	  ...
	  function varargout = makeTraceFromVid(obj, vid)
		 % TODO: apply pixel weights
		 % Get Reshaped Video-Array from Vid Input
		 if isstruct(vid)
			f = makeTraceFromVidLowMem(obj,vid);
			if nargout > 0
			   varargout{1} = f;
			end
			return
...
% 	  end
   ...
   methods % PRIVATE MANAGEMENT FUNCTIONS
	  function varargout = guessFrameSize(obj)
		 ...
		 if numel(obj) > 1
            ...
            fprintf(['Assuming frame size ',num2str(fsz),'\n']);
	  ...
	  function roiClickFcn(obj,src,evnt)		 
		 ...
		 vp = .65;
		 sz = get(0,'ScreenSize');
		 % 		 clickPoint = fliplr(round(evnt.IntersectionPoint(1:2)));
		 ...
		 if isempty(hAx) || ~isvalid(hAx)
			if sz(4) > sz(3) % (vertical)
			   p = vp - .1;
			   hAx = axes('Position',[.01, p, .98, .99-p],'Parent',hFig);
			else				% (horizontal)
			   p = hp;
   ...
   methods (Access = protected) % CUSTOMDISPLAY
	  % 	  function propgrp = getPropertyGroups(~)
	  % 		 proplist = {'Idx','Area','BoundingBox','Centroid','Eccentricity',...
	  % 			'Perimeter','XLim','YLim','Width','Height',...
	  % 			'PixelIdxList','PixelSubScripts',...
	  % 			'PixelWeights','Frames','SubRegion','SuperRegion',...
	  % 			'Trace','Color'};
	  % 		 propgrp = matlab.mixin.util.PropertyGroup(proplist);
	  % 	  end
   end
   methods % Hua-an
	  function varargout = makeTraceFromVidLowMemWithoutWeight(obj, vid)
		 % Note on timing for making traces: The following times were recorded
		 % for video input as a structure array, then a numeric array. (8188
		 % frames, 67 ROIs).
		 % vid       1x8188            8668333156  struct
...
% 		 end
	  ...
	  function varargout = makeTraceFromVidWithoutWeight(obj, vid)
		 % TODO: apply pixel weights
		 % Get Reshaped Video-Array from Vid Input
		 if isstruct(vid)
			f = makeTraceFromVidLowMemWithoutWeight(obj,vid);
			if nargout > 0
			   varargout{1} = f;
			end
			return
...
% 		 end
      ...
      function varargout = makeBackgroundTraceFromVidLowMem(obj, vid)
		 % Note on timing for making traces: The following times were recorded
		 % for video input as a structure array, then a numeric array. (8188
		 % frames, 67 ROIs).
		 % vid       1x8188            8668333156  struct
		 ...
		 nObj = numel(obj);
		 if nObj > 1
			nIdx = cat(1,obj.BackgroundArea);
			maxIdx = max(nIdx);
			idxByColumn = zeros([maxIdx,nObj],'double');
			for k=1:numel(obj)
			   idxByColumn(1:nIdx(k), k) = obj(k).BackgroundPixelIdxList(:);
            end
		 else
			nIdx = obj.BackgroundArea;
		 end
		 % Get Reshaped Video-Array from Vid Input
		 if isstruct(vid)
			nFrames = numel(vid);
...
% 		 f = bsxfun(@rdivide, bsxfun(@minus, f, nanmean(fnan,1)), nanvar(fnan,1));
		 % ASSIGN TRACE TO ROIs
% 		 for kRoi = 1:nObj
% 			obj(kRoi).BackgroundTrace = f(:,kRoi);
% 		 end
		 if nargout > 0
			varargout{1} = f;
		 end
	  end
	  function varargout = makeBackgroundTraceFromVid(obj, vid)
		 % TODO: apply pixel weights
		 % Get Reshaped Video-Array from Vid Input
		 if isstruct(vid)
			f = makeBackgroundTraceFromVidLowMem(obj,vid);
			if nargout > 0
			   varargout{1} = f;
			end
			return
...
% 		 end
      ...
      function varargout = show_all_background_mask(obj)
         all_background_mask = zeros(obj(1).FrameSize);
         all_background_mask(cat(1,obj.BackgroundPixelIdxList)) = 1;
         imagesc(all_background_mask);
         axis image off;
         fprintf(['Background Range: ',num2str(mean(cat(1,obj.BackgroundRange))),' pixel(s).\n']);
      end
      function varargout = compare_traces(obj)
          fprintf(['Background Range: ',num2str(mean(cat(1,obj.BackgroundRange))),' pixel(s).\n']);
          fprintf('Trace (weighted): Blue\n');
          fprintf('Raw Trace (unweighted): Red\n');
          fprintf('Background Trace: Yellow\n');
          fprintf('Raw Trace - Background (unweighted): Green\n');
          for i=1:numel(obj)
              plot(obj(i).TraceTime,obj(i).Trace,'b');
              hold on;
              plot(obj(i).TraceTime,obj(i).RawTraceWithoutWeight,'r');
              plot(obj(i).TraceTime,obj(i).BackgroundTrace,'y');
              plot(obj(i).TraceTime,obj(i).RefinedTrace,'g');
              hold off;
              title(['ROI ',num2str(obj(i).id),' / ROI area: ',num2str(obj(i).Area),' / Background area: ',num2str(obj(i).BackgroundArea)]);
              pause;
          end
          
      end
      function varargout = compare_traces_isolated(obj)
          compare_traces(findobj(obj,'isIsolate',true));
      end
      function varargout = check_framesize(obj)
          fprintf('Checking the frame size in all ROIs....\n');
          if numel(unique([obj.FrameSize]))~=1 || ~isempty(findobj(obj,'FrameSize',[]))
              fprintf('Inconsistant frame size!\n');
              fprintf('Try to fix frame size....\n');
              obj.fix_framesize;
          elseif numel(unique([obj.FrameSize]))==1 && max(cat(1,obj.PixelIdxList))>unique([obj.FrameSize])^2
              fprintf('Inconsistant frame size!\n');
              fprintf('Try to fix frame size....\n');
              obj.fix_framesize;
          else
              fprintf('The frame size in all ROIs are consistant. :)\n');
          end
      end
      function varargout = fix_framesize(obj)
          current_fsz = mode([obj.FrameSize]); % find the most common frame size
          maxidx = max(cat(1,obj.PixelIdxList));
          sqsize = 2.^(1:12);
          framePow2 = find(sqsize > sqrt(maxidx),1,'first');
          pixel_fsz = 2^framePow2; % guess the frame size from pixel index

          if current_fsz == pixel_fsz
              fsz = current_fsz;    
          else
              fprintf('Fail to fix frame size\n');
              fprintf(['Frame size based on dataset information: ',num2str(current_fsz),'\n']);
              fprintf(['Frame size based on pixel index: ',num2str(pixel_fsz),'\n']);
              input_fsz = input(['Please input frame size: ']);
              fsz = input_fsz;
          end
          set(obj,'FrameSize',[fsz fsz]); 
          fprintf(['Set all frame size to ',num2str(fsz),'.\n']);
      end
   end
